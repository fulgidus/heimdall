# Step 3: Synthetic IQ Sample Generator

## Objective

Implement `SyntheticIQGenerator` to generate realistic 1000ms IQ samples for each receiver, simulating multipath propagation, Rayleigh fading, and AWGN.

## Context

Currently, synthetic data generation creates high-level features (SNR, PSD) directly. We need to generate **raw complex IQ samples** first, then extract features from them.

This enables:
- Realistic temporal dynamics (fading, multipath)
- Training deep learning models on raw IQ data
- Consistency with real recording sessions

## Implementation

### 1. Create IQ Generator Module
...
iq = generator.generate_iq_sample(
    center_frequency_hz=144_000_000,
    signal_power_dbm=-65.0,
    noise_floor_dbm=-87.0,
    snr_db=20.0,
...
print(f"Generated {len(iq.samples)} samples")
print(f"Sample type: {iq.samples.dtype}")
print(f"Power: {10 * np.log10(np.mean(np.abs(iq.samples)**2)):.1f} dBm")
```

Expected output:
```
Generated 200000 samples
Sample type: complex64
Power: -65.0 dBm (approximately)
```

## Success Criteria

- ✅ `SyntheticIQGenerator` class implemented
- ✅ Generates complex64 IQ samples with correct length
- ✅ Multipath adds 2-3 delayed reflections
- ✅ Rayleigh fading varies envelope over 50-200ms
- ✅ AWGN achieves target SNR
...
Proceed to **`04-synthetic-pipeline.md`** to integrate IQ generation into the synthetic data pipeline.
