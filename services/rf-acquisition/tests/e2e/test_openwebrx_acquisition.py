...
class TestOpenWebRXAcquisition:
    ...
    async def test_acquisition_single_frequency(self, api_client: AsyncClient):
        ...
        while elapsed < max_wait:
            ...
            response = await api_client.get(
                f"/api/v1/acquisition/status/{task_id}"
            )
        ...
        assert measurements_collected == 7, \
            f"Expected 7 measurements, got {measurements_collected}"
        
        print(f"✅ All 7 WebSDRs collected data")
        ...
        for idx, measurement in enumerate(result["measurements"], 1):
            ...
            print(f"  ✅ WebSDR {idx}: SNR={metrics['snr_db']:.2f} dB, "
    ...
    async def test_acquisition_multiple_frequencies(self, api_client: AsyncClient):
        ...
        for freq in frequencies:
            ...
            results.append({
                "frequency": freq,
                "measurements": task_status["measurements_collected"]
            })
    ...
    async def test_concurrent_acquisitions(self, api_client: AsyncClient):
        ...
        for freq, task_id in task_ids:
            ...
            task_status = response.json()
            assert task_status["status"] == "SUCCESS"
            assert task_status["measurements_collected"] >= 5, \
            ...
            for attempt in range(20):
                try:
                    ...
                    if isinstance(msg, bytes):
                        ...
                        print(f"✅ Binary IQ data received: {len(msg)} bytes (after {messages_received} messages)")
                        break
                    else:
