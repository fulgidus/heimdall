...
class WebSDRFetcher:
    ...
    async def __aenter__(self):
        ...
        connector = aiohttp.TCPConnector(
            limit=self.concurrent_limit, limit_per_host=2, ttl_dns_cache=300
        )
    ...
    async def fetch_iq_simultaneous(
    ...
    ) -> dict[int, tuple[np.ndarray, str | None]]:
        """
        ...
        tasks = [
            self._fetch_from_websdr(
                websdr_id, websdr_cfg, frequency_hz, sample_rate_hz, expected_samples
            )
        ...
        for (websdr_id, _), result in zip(
            sorted(self.websdrs.items(), key=lambda x: x[0]), results, strict=False
        ):
            iq_data, error = result
            iq_data_dict[websdr_id] = (iq_data, error)

            if error:
                logger.warning("Failed to fetch from WebSDR %d: %s", websdr_id, error)
            else:
                logger.info(
    ...
    async def _fetch_from_websdr(
        ...
        async with self.semaphore:
            for attempt in range(self.retry_count):
                try:
                    return (
                        await self._fetch_single_attempt(
                            websdr_cfg, frequency_hz, sample_rate_hz, expected_samples
                        ),
                        None,
                    )
    ...
    async def _fetch_single_attempt(
        ...
        async with self.session.get(url, params=params) as response:
            if response.status != 200:
                raise RuntimeError(f"HTTP {response.status} from {websdr_cfg.name}")
            ...
            logger.debug("Received %d samples from %s", len(iq_complex), websdr_cfg.name)
        ...
        async def check_single(websdr_id: int, websdr_cfg: WebSDRConfig) -> tuple[int, bool]:
            ...
            try:
                ...
                async with self.session.head(
                    str(websdr_cfg.url), timeout=timeout, allow_redirects=False
                ) as response:
                    # 501 = "Method Not Supported" - server is alive but doesn't support HEAD
                    if response.status == 501:
                        logger.debug(
                            f"  → 501 (HEAD not supported), trying GET... (SDR #{websdr_id})"
                        )
                    ...
                    # 4xx/5xx (except 501) = Error
                        ...
                        logger.warning(
                            f"  → {response.status} Error (SDR #{websdr_id} appears OFFLINE)"
                        )
                ...
                logger.warning(
                    f"Health check timeout for {websdr_cfg.url} (SDR #{websdr_id}) - marking OFFLINE"
                )
                return websdr_id, False
            except Exception as e:
                logger.warning(
                    f"Health check error for {websdr_cfg.url} (SDR #{websdr_id}): {type(e).__name__}: {e} - marking OFFLINE"
                )
                return websdr_id, False

        tasks = [check_single(wsid, wscfg) for wsid, wscfg in self.websdrs.items()]
