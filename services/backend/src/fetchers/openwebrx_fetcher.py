...
class OpenWebRXClient:
            ...
            while True:
                ...
                if isinstance(message, bytes):
                    ...
                    if samples is not None and len(samples) > 0:
                        ...
                        logger.debug(
                            f"Received {len(samples)} samples, total: {sum(len(s) for s in iq_samples)}"
                        )
    ...
    def _parse_iq_data(self, data: bytes) -> np.ndarray | None:
        """
...
class OpenWebRXFetcher:
    ...
    async def fetch_iq_simultaneous(
    ...
    ) -> dict[int, tuple[np.ndarray | None, str | None]]:
        """
        ...
        tasks = [
            self._fetch_from_websdr(websdr_id, websdr_cfg, frequency_hz, duration_seconds)
            ...
            for attempt in range(self.retry_count):
                try:
                    async with OpenWebRXClient(websdr_cfg, self.timeout) as client:
                        iq_data = await client.fetch_iq_data(frequency_hz, duration_seconds)
                        return iq_data, None
    ...
    async def health_check(self) -> dict[int, dict]:
        ...
        async def check_single(websdr_id: int, websdr_cfg: WebSDRConfig) -> tuple[int, dict]:
            ...
            try:
                ...
                async with OpenWebRXClient(websdr_cfg, timeout=5):
                    response_time_ms = (time.time() - start_time) * 1000
                    logger.info(
                        f"WebSDR {websdr_id} ({websdr_cfg.name}) is ONLINE (response: {response_time_ms:.1f}ms)"
                    )
                    return websdr_id, {
                        "online": True,
                ...
                logger.warning(
                    f"Health check error for WebSDR {websdr_id}: {type(e).__name__}: {e} - marking OFFLINE"
                )
                return websdr_id, {"online": False, "response_time_ms": None}
