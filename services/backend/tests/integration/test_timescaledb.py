...
def test_measurement_creation_from_dict():
    ...
    measurement = Measurement.from_measurement_dict(
        task_id="test-session-001", measurement_dict=sample_dict, s3_path="s3://bucket/test.npy"
    )
...
def test_measurement_to_dict():
    ...
    measurement = Measurement.from_measurement_dict(
        task_id="test-session-001", measurement_dict=sample_dict
    )
...
def test_measurement_missing_required_field():
    ...
    with pytest.raises(ValueError):
        Measurement.from_measurement_dict(task_id="test", measurement_dict=invalid_dict)
...
def test_insert_single_measurement():
    ...
    meas_id = manager.insert_measurement(
        task_id=task_id, measurement_dict=sample_dict, s3_path="s3://bucket/websdr_1.npy"
    )
...
def test_bulk_insert_measurements():
    ...
    successful, failed = manager.insert_measurements_bulk(
        task_id=task_id, measurements_list=measurements_list
    )
...
def test_get_snr_statistics():
    ...
    manager.insert_measurements_bulk(task_id=task_id, measurements_list=measurements_list)
