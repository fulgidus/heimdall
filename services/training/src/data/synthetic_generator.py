...
def _generate_hexagonal_receivers(
    num_receivers: int,
    area_lat_min: float,
    area_lat_max: float,
    area_lon_min: float,
    ...
    """
    Generate receivers in hexagonal lattice pattern for optimal GDOP.
    
    Hexagonal lattice provides:
    - Uniform spatial coverage
    - Minimal GDOP (typically 8-15 vs 100+ for random)
    - Even angular distribution around transmitters
    
    Algorithm:
    1. Calculate optimal spacing for requested receiver count
    2. Generate hexagonal lattice grid
    3. Add ±10% jitter to prevent overfitting to perfect geometry
    4. Clip to area bounds
    
    Args:
        num_receivers: Target number of receivers (actual may differ slightly)
        area_lat_min: Minimum latitude
        area_lat_max: Maximum latitude
        area_lon_min: Minimum longitude
        area_lon_max: Maximum longitude
    ...
    lat_range = area_lat_max - area_lat_min
    lon_range = area_lon_max - area_lon_min
    area = lat_range * lon_range
    
    # Hexagonal lattice packing efficiency: each cell area = spacing^2 * sqrt(3)/2
    # Solve for spacing given target receiver count:
    # num_receivers ≈ area / (spacing^2 * sqrt(3)/2)
    # spacing = sqrt(area / (num_receivers * sqrt(3)/2))
    spacing = np.sqrt(area / (num_receivers * np.sqrt(3) / 2))
    ...
    while y <= area_lat_max and len(receivers) < num_receivers * 1.5:  # Generate extra, then trim
        # Hexagonal offset: every other row shifts by half spacing
        x_offset = (spacing / 2) if (row % 2 == 1) else 0
        x = area_lon_min + x_offset
        
        while x <= area_lon_max:
            # Add jitter (±10% of spacing) to prevent overfitting to perfect geometry
            jitter_lat = rng.uniform(-0.1 * spacing, 0.1 * spacing)
            jitter_lon = rng.uniform(-0.1 * spacing, 0.1 * spacing)
            
            lat = y + jitter_lat
            lon = x + jitter_lon
            
            # Keep within bounds
            if area_lat_min <= lat <= area_lat_max and area_lon_min <= lon <= area_lon_max:
                # Get altitude from terrain if available
                if terrain_lookup:
                    try:
                        alt = terrain_lookup.get_elevation(lat, lon)
                        # Add 10-20m above ground for antenna height
        ...
        # Hexagonal vertical spacing: spacing * sqrt(3)/2
        y += spacing * np.sqrt(3) / 2
        row += 1
    
    # Trim to exactly num_receivers (or close to it)
    # If we generated too few, that's OK - better geometry is worth it
    if len(receivers) > num_receivers:
        # Randomly sample to get desired count (maintains spatial distribution)
        indices = rng.choice(len(receivers), size=num_receivers, replace=False)
        receivers = [receivers[i] for i in sorted(indices)]
    
    logger.info(
        "Generated hexagonal receiver lattice",
        target_receivers=num_receivers,
        actual_receivers=len(receivers),
        spacing=f"{spacing:.4f}°",
        area_sq_deg=f"{area:.4f}",
        jitter_percent=10.0
    )
...
def _generate_single_sample_no_features(args):
    ...
    if use_random_receivers:
        ...
        # Select receiver placement strategy
        placement_strategy = config.get('receiver_placement_strategy', 'hexagonal')
        
        if placement_strategy == 'hexagonal':
            receivers_list = _generate_hexagonal_receivers(
            ...
            logger.debug(f"Sample {sample_idx}: Generated {len(receivers_list)} hexagonal receivers")
    ...
    propagation_metadata = {
        ...
        'signal_powers_dbm': signal_powers_dbm.tolist(),  # RF features for batch processing
        'frequency_offsets': frequency_offsets.tolist(),
...
def _generate_single_sample(args):
    ...
    for i, features_dict in enumerate(features_dicts):
        ...
        # Add RF features (PSD and frequency offset)
        # PSD = signal_power / bandwidth (in dB: PSD_dBm/Hz = Power_dBm - 10*log10(BW_Hz))
        bandwidth_hz = bandwidths[i]  # 12.5 kHz
        signal_power_dbm = signal_powers_dbm[i]
        psd_dbm_hz = signal_power_dbm - 10 * np.log10(bandwidth_hz)
        features_dict['psd_dbm'] = float(psd_dbm_hz)
        features_dict['freq_offset_hz'] = float(frequency_offsets[i])
    ...
    while valid_samples_collected < target_valid_samples and total_attempted < max_attempts:
        ...
        for raw_result in batch_raw_results:
            ...
            for i, features_dict in enumerate(sample_features_dicts):
                ...
                # Add RF features (PSD and frequency offset) from propagation metadata
                signal_powers_dbm = raw_result['propagation_metadata']['signal_powers_dbm']
                frequency_offsets = raw_result['propagation_metadata']['frequency_offsets']
                bandwidths = raw_result['propagation_metadata']['bandwidths']
                
                signal_power_dbm = signal_powers_dbm[i]
                bandwidth_hz = bandwidths[i]
                psd_dbm_hz = signal_power_dbm - 10 * np.log10(bandwidth_hz)
                features_dict['psd_dbm'] = float(psd_dbm_hz)
                features_dict['freq_offset_hz'] = float(frequency_offsets[i])
        ...
        if valid_in_batch == 0:
            ...
            if consecutive_failures >= max_consecutive_failures and not disable_safety_checks:
                ...
                break
            elif consecutive_failures >= max_consecutive_failures and disable_safety_checks:
                logger.warning(f"Safety checks disabled: Continuing despite {consecutive_failures} consecutive failures. Use with caution!")
        else:
    ...
    for sample in samples:
        ...
        for feat in sample['receiver_features']:
            ...
            receivers_metadata.append({
                ...
                'psd_dbm': float(feat['psd_dbm']),  # RF feature - NO hardcoded fallback
                'freq_offset_hz': float(feat['freq_offset_hz']),  # RF feature - NO hardcoded fallback
