...
class HuberNLLLoss(nn.Module):
    ...
    def __init__(self, delta: float = 1.0, reduction: str = 'mean'):
        super(HuberNLLLoss, self).__init__()
        self.delta = delta
        self.reduction = reduction
        self.eps = 1e-6
    ...
    def forward(
        ...
        huber_loss = F.huber_loss(normalized_residuals, torch.zeros_like(normalized_residuals), 
...
def verify_gaussian_nll_loss():
    ...
    assert loss_low_sigma > loss_high_sigma, \
