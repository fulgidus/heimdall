        ...
        with torch.no_grad():
            for _ in range(num_batches):
                ...
                onnx_outputs = sess.run(
                    None, {"mel_spectrogram": test_input_np}  # Output names = all outputs
                )
    ...
    def register_with_mlflow(
        ...
        try:
            ...
            self.mlflow_tracker.log_artifact(
                metadata_json, artifact_path=f"models/{model_name}/metadata.json"
            )
...
if __name__ == "__main__":
    ...
    logger.info(
        f"✅ ONNX export successful! Inputs: {model_info['inputs']}, Outputs: {model_info['outputs']}"
    )
