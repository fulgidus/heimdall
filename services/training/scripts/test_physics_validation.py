...
Run from container:
docker compose exec training python /app/scripts/test_physics_validation.py
"""
...
class PhysicsValidator:
    ...
    def check_monotonicity_violation(
        ...
        Only considers receivers that actually detected the signal (SNR > -500).
        ...
        # Filter out receivers without signal (SNR = -999.0)
        receivers_with_signal = [r for r in receivers_data if r['snr_db'] > -500.0]
        
        # Need at least 2 receivers with signal to check monotonicity
        if len(receivers_with_signal) < 2:
            return False, []
        
        # Sort by distance
        sorted_receivers = sorted(receivers_with_signal, key=lambda r: r['distance_km'])
    ...
    def analyze_sample(self, sample) -> Dict:
        """
        Analyze a single sample for physics anomalies.
        
        Args:
            sample: Generated SyntheticSample dataclass
        ...
        # Extract receiver data from dataclass (sample.receivers is a list of dicts)
        receivers_data = []
        for rx_idx, rx in enumerate(sample.receivers):
            details = rx.get('details', {})
            
            receivers_data.append({
                'idx': rx_idx,
                'distance_km': details.get('distance_km', rx.get('distance_km', 0)),
                'snr_db': rx.get('snr', -999.0),
                'details': details
            })
...
def main():
    ...
    print("Generating 1000 samples in batches of 100...")
    ...
    for batch_idx in range(num_batches):
        print(f"  Processing batch {batch_idx + 1}/{num_batches} (samples {batch_idx * batch_size + 1}-{(batch_idx + 1) * batch_size})...")
        
        try:
            # Generate batch of samples
            samples = generator.generate_samples(num_samples=batch_size)
