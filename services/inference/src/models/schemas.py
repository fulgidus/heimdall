...
class PredictionRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None,
        description="Optional session ID for tracking and audit trail"
    )
    cache_enabled: bool = Field(
        True,
        description="Enable Redis caching for this prediction"
    )
...
class UncertaintyResponse(BaseModel):
    ...
    sigma_x: float = Field(
        ...,
        description="Standard deviation in X direction (meters)",
        ge=0.0
    )
    sigma_y: float = Field(
        ...,
        description="Standard deviation in Y direction (meters)",
        ge=0.0
    )
    theta: float = Field(
        0.0,
        description="Rotation angle of ellipse in degrees (-180 to 180)",
        ge=-180.0,
        le=180.0
    )
    confidence_interval: float = Field(
        0.68,
        description="Confidence interval (1-sigma = 68%, 2-sigma = 95%, etc.)",
        ge=0.0,
        le=1.0
    )
...
class PredictionResponse(BaseModel):
    ...
    position: PositionResponse = Field(
        ...,
        description="Predicted position"
    )
    ...
    confidence: float = Field(
        ...,
        description="Model confidence (0-1, higher is more confident)",
        ge=0.0,
        le=1.0
    )
    model_version: str = Field(
        ...,
        description="Version of model used for this prediction"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "uncertainty": {"sigma_x": 45.0, "sigma_y": 38.0, "theta": 25.0, "confidence_interval": 0.68},
                "confidence": 0.92,
...
class ModelInfoResponse(BaseModel):
    ...
    accuracy_meters: Optional[float] = Field(
        None,
        description="Localization accuracy (sigma) in meters from training"
    )
    training_samples: Optional[int] = Field(None, description="Number of samples in training set")
    ...
    status: str = Field(
        ...,
        description="Service status (ready, loading, error)"
    )
