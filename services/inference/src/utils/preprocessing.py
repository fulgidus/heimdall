...
class IQPreprocessor:
    ...
    def _build_mel_filterbank(self) -> np.ndarray:
        ...
        """
        # Nyquist frequency
        nyquist = self.config.f_max
        
        # Convert frequency range to FFT bins
        n_fft_bins = self.config.n_fft // 2 + 1
        f_min_bin = int(np.ceil(self.config.f_min * n_fft_bins))
        f_max_bin = int(np.floor(self.config.f_max * n_fft_bins))
        ...
        # Convert to FFT bins
        bin_points = np.array([
            int(np.floor(f * n_fft_bins)) for f in freq_points
        ])
        ...
        for m in range(self.config.n_mels):
            ...
            if center > left:
                filterbank[m, left:center] = (
                    np.arange(center - left) / (center - left)
                )
            
            # Right slope
            if right > center:
                filterbank[m, center:right] = (
                    np.arange(right - center, 0, -1) / (right - center)
                )
    ...
    def _apply_log_scale(self, mel_spec: np.ndarray, epsilon: float = 1e-10) -> np.ndarray:
        ...
        logger.debug(f"Applied log scaling: min={mel_spec_log.min():.3f}, max={mel_spec_log.max():.3f}")
        return mel_spec_log
    ...
    def _normalize(self, mel_spec_log: np.ndarray) -> np.ndarray:
        ...
        logger.debug(
            f"Normalized: mean={normalized.mean():.6f}, std={normalized.std():.6f}"
        )
        return normalized
