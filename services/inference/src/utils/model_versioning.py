...
class ABTestConfig:
    ...
    duration_seconds: int | None = None  # None = unlimited
...
class ModelVersionRegistry:
    ...
    def __init__(
        ...
        self.previous_version_id: str | None = None  # For fallback
        ...
        async with self._lock:
            ...
            try:
                ...
                session = ort.InferenceSession(
                    model_path, self.session_options, providers=["CPUExecutionProvider"]
                )
    ...
    def end_ab_test(self, winner: str | None = None) -> bool:
        """
    ...
    async def predict(
        self, features: np.ndarray, use_ab_routing: bool = True
...
async def create_version_registry(
    mlflow_uri: str = None, initial_versions: dict[str, str] | None = None
    ...
    if initial_versions:
        for version_id, model_path in initial_versions.items():
            await registry.load_version(
                version_id, stage=ModelStage.PRODUCTION, model_path=model_path
            )
