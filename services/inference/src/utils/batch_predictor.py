...
class BatchPredictionRequest(BaseModel):
    ...
    iq_samples: List[BatchIQDataItem] = Field(
        ...,
        description="List of IQ samples to predict",
        min_items=1,
        max_items=100
    )
    ...
    continue_on_error: bool = Field(
        default=True,
        description="Continue processing even if some samples fail"
    )
        ...
        schema_extra = {
            "example": {
                "iq_samples": [
                    {
                        "sample_id": "s1",
                        "iq_data": [[1.0, 0.5], [1.1, 0.4], [1.2, 0.6]]
                    },
                    {
                        "sample_id": "s2",
                        "iq_data": [[0.9, 0.6], [0.8, 0.5], [1.0, 0.7]]
                    }
                ],
                "cache_enabled": True,
...
class BatchPredictionItemResponse(BaseModel):
    ...
    sample_id: str
    success: bool
    position: Optional[Dict[str, float]] = None  # {"lat": X, "lon": Y}
    uncertainty: Optional[Dict[str, float]] = None  # {"sigma_x": X, "sigma_y": Y, "theta": Z}
            ...
            async with self._semaphore:  # Bounded concurrency
                ...
                if cache_enabled and self.cache:
                    try:
                        ...
                        cache_key_bytes = mel_spec.tobytes()
