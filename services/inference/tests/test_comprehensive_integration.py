...
class TestModelVersioningIntegration:
    ...
    async def test_version_loading_and_switching(self):
        ...
        v1_loaded = await registry.load_version(
            ...
            model_path="tests/fixtures/model_v1.onnx"
        )
...
class TestBatchPredictionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_batch_prediction_success(self, mock_model_loader, mock_redis_cache, 
                                            iq_preprocessor, mock_metrics_manager):
        ...
        samples = [
            BatchIQDataItem(
                sample_id=f"s{i}",
                iq_data=np.random.randn(1024, 2).tolist()
            )
            for i in range(3)
        ]
    ...
    @pytest.mark.asyncio
    async def test_batch_prediction_latency_sla(self, mock_model_loader, mock_redis_cache,
                                                iq_preprocessor, mock_metrics_manager):
        ...
        # Check SLA
        assert response.p95_latency_ms < 500, \
            f"P95 latency {response.p95_latency_ms}ms exceeds 500ms SLA"

    @pytest.mark.asyncio
    async def test_batch_prediction_error_recovery(self, mock_model_loader, mock_redis_cache,
                                                   iq_preprocessor, mock_metrics_manager):
    ...
    @pytest.mark.asyncio
    async def test_batch_prediction_concurrent_limit(self, mock_model_loader, mock_redis_cache,
                                                     iq_preprocessor, mock_metrics_manager):
        ...
        samples = [
            BatchIQDataItem(
                sample_id=f"s{i}",
                iq_data=np.random.randn(512, 2).tolist()
            )
            for i in range(10)
        ]
...
class TestEndToEndInferenceWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_single_prediction_workflow(self, iq_preprocessor, mock_model_loader,
                                              mock_redis_cache, sample_iq_data):
    ...
    @pytest.mark.asyncio
    async def test_batch_prediction_with_caching_workflow(self, iq_preprocessor,
                                                          mock_model_loader,
                                                          mock_redis_cache,
        ...
        assert response.total_time_ms > 0
        assert all(
            isinstance(p.inference_time_ms, float) 
    ...
    async def test_versioning_with_fallback_workflow(self):
        ...
        # Simulate loading v1
        v1_meta = {"version": "v1", "accuracy": 0.94}
...
class TestPerformanceAndSLAValidation:
    ...
    @pytest.mark.asyncio
    async def test_single_prediction_latency_sla(self, iq_preprocessor, mock_model_loader,
                                                 sample_iq_data):
    ...
    @pytest.mark.asyncio
    async def test_throughput_sla_validation(self, mock_model_loader, mock_redis_cache,
                                             iq_preprocessor, mock_metrics_manager):
...
class TestErrorHandlingAndRecovery:
    ...
    @pytest.mark.asyncio
    async def test_model_unavailable_recovery(self, mock_model_loader, mock_redis_cache,
                                              iq_preprocessor, mock_metrics_manager):
        ...
        request = BatchPredictionRequest(
            iq_samples=samples,
            continue_on_error=True  # Should handle error gracefully
        )
    ...
    @pytest.mark.asyncio
    async def test_cache_connection_failure_recovery(self, mock_model_loader, iq_preprocessor,
                                                     mock_metrics_manager):
...
@pytest.mark.asyncio
async def test_batch_sizes(batch_size, mock_model_loader, mock_redis_cache,
                           iq_preprocessor, mock_metrics_manager):
