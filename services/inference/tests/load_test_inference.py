    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config": {
                ...
                "total_target_requests": self.config.concurrent_users
...
class InferenceLoadTester:
    ...
    async def run_user_session(
        ...
        for request_idx in range(self.config.requests_per_user):
            ...
            # Generate IQ data
            np.random.randn(self.config.iq_sample_size, 2).tolist()
            ...
            try:
                ...
                if np.random.random() < (
                    self.config.cache_hit_rate_target if request_idx > 0 else 0
                ):
                    # Simulated cache hit
            ...
            logger.debug(
                f"User {user_id}: request {request_idx} completed in {metrics.duration_ms:.1f}ms"
            )
    ...
    async def run(self) -> LoadTestResults:
        ...
        logger.info(
            f"P95 latency: {results.p95_latency:.1f}ms (SLA: ≤{self.config.p95_latency_ms:.0f}ms)"
        )
        logger.info(
            f"Cache hit rate: {results.cache_hit_rate:.1%} (Target: ≥{self.config.cache_hit_rate_target:.0%})"
        )
