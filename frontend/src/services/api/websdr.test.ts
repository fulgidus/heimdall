...
import { getWebSDRs, checkWebSDRHealth, getWebSDRConfig, getActiveWebSDRs } from './websdr';
...
describe('WebSDR API Service', () => {
  ...
  describe('getWebSDRConfig', () => {
    ...
    it('should throw error when WebSDR not found', async () => {
      await expect(getWebSDRConfig('550e8400-e29b-41d4-a716-446655449999')).rejects.toThrow(
        'WebSDR with id 550e8400-e29b-41d4-a716-446655449999 not found'
      );
    });

    it('should throw error when id is invalid', async () => {
      await expect(getWebSDRConfig('invalid-uuid')).rejects.toThrow(
        'WebSDR with id invalid-uuid not found'
      );
    });
  });
