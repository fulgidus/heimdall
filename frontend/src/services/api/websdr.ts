...
export async function getWebSDRs(): Promise<WebSDRConfig[]> {
    console.log('📡 WebSDRService.getWebSDRs(): calling GET /v1/acquisition/websdrs-all');
...
export async function checkWebSDRHealth(): Promise<Record<string, WebSDRHealthStatus>> {
    console.log(
        '🏥 WebSDRService.checkWebSDRHealth(): calling GET /v1/acquisition/websdrs/health'
    );
 ...
 */
export async function fetchWebSDRInfo(
    url: string
): Promise<import('./schemas').WebSDRFetchInfoResponse> {
    ...
    console.log(
        '✅ WebSDRService.fetchWebSDRInfo(): received',
