...
describe('RecordingSession Page', () => {
  ...
  it('renders RecordingSession page with title', () => {
    ...
    expect(
      screen.queryAllByText(/RF Source/i).length > 0 ||
        screen.queryAllByText(/Recording|Acquisition/i).length > 0
    ).toBe(true);
  });
  ...
  it('displays WebSDR status', () => {
    render(<RecordingSession />);
    expect(
      screen.queryAllByText(/online|websdr/i).length > 0 ||
        screen.queryAllByText(/recording|session/i).length > 0
    ).toBe(true);
  });
  ...
  it('displays online WebSDR count', () => {
    render(<RecordingSession />);
    expect(
      screen.queryAllByText(/online|websdr|7/i).length > 0 ||
        screen.queryAllByText(/recording/i).length > 0
    ).toBe(true);
  });
