...
export const useAcquisitionStore = create<AcquisitionStore>((set, get) => ({
  ...
  pollTask: async (taskId: string, onProgress?: (status: AcquisitionStatusResponse) => void) => {
    const finalStatus = await acquisitionService.pollAcquisitionStatus(taskId, status => {
      get().updateTaskStatus(taskId, status);
      onProgress?.(status);
    });
