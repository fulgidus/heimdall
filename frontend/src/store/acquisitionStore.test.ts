...
describe('Acquisition Store (Zustand)', () => {
  ...
  describe('getTaskStatus Action', () => {
    ...
    it('should handle get status error', async () => {
      ...
      await expect(useAcquisitionStore.getState().getTaskStatus('task-999')).rejects.toThrow(
        errorMessage
      );
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle concurrent acquisitions', async () => {
      vi.mocked(acquisitionService.triggerAcquisition).mockImplementation(async request => ({
