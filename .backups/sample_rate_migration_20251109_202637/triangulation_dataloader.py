...
def collate_iq_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    ...
    # Stack RF features (SNR, PSD, freq_offset)
    receiver_snr = torch.stack([sample["receiver_snr"] for sample in batch])
    receiver_psd = torch.stack([sample["receiver_psd"] for sample in batch])
    receiver_freq_offset = torch.stack([sample["receiver_freq_offset"] for sample in batch])
    ...
    return {
        ...
        "receiver_psd": receiver_psd,
        "receiver_freq_offset": receiver_freq_offset,
        "metadata": metadata
    }
...
class TriangulationIQDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            receiver_positions = []
            signal_mask = []
            receiver_snr_list = []  # Extract SNR from metadata
            receiver_psd_list = []  # Extract PSD (if available)
            receiver_freq_offset_list = []  # Extract freq offset (if available)
            ...
            for rx_meta in receivers_metadata:
                ...
                # Extract RF features from metadata
                snr_db = rx_meta.get('snr_db', 0.0)
                psd_dbm = rx_meta.get('psd_dbm', -80.0)  # Default if not present
                freq_offset_hz = rx_meta.get('freq_offset_hz', 0.0)  # Default if not present
                ...
                # Append RF features to lists
                receiver_snr_list.append(snr_db)
                receiver_psd_list.append(psd_dbm)
                receiver_freq_offset_list.append(freq_offset_hz)
            ...
            if current_receivers < self.max_receivers:
                ...
                # Add zero IQ samples and pad RF features
                for _ in range(padding_count):
                    ...
                    receiver_snr_list.append(0.0)  # Pad with zero SNR
                    receiver_psd_list.append(-80.0)  # Pad with default PSD
                    receiver_freq_offset_list.append(0.0)  # Pad with zero offset
            ...
            # Convert RF features to tensors
            receiver_snr = torch.tensor(receiver_snr_list, dtype=torch.float32)  # (num_receivers,)
            receiver_psd = torch.tensor(receiver_psd_list, dtype=torch.float32)  # (num_receivers,)
            receiver_freq_offset = torch.tensor(receiver_freq_offset_list, dtype=torch.float32)  # (num_receivers,)
