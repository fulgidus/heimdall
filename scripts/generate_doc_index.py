...
class DocumentationIndexer:
    ...
    def resolve_link(self, url: str, source_file: Path) -> Tuple[Optional[Path], Optional[str]]:
        """
    ...
    def extract_and_validate_links(self, file_path: Path):
        ...
        for match in re.finditer(pattern, content):
            ...
            if validation['valid']:
                ...
                self.log(f"Broken link: [{link_text}]({url}) - {validation.get('error', 'Unknown error')}", 'warning')
        
        self.all_links[file_path] = links_in_file
        self.log(f"  Found {len(links_in_file)} links ({len([l for l in links_in_file if l['valid']])} valid)")
...
Exit Codes:
  ...
  1 = Broken links detected
    ...
    parser.add_argument(
        '--check-anchors',
        action='store_true',
        help='Validate anchor targets in links'
    )
    parser.add_argument(
        '--sitemap',
        action='store_true',
        help='Generate visual sitemap'
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
    ...
    indexer = DocumentationIndexer(base_path, check_anchors=args.check_anchors, verbose=args.verbose)
