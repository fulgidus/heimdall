...
class PerformanceBenchmark:
    ...
    def benchmark_api_endpoints(self) -> None:
        ...
        for method, endpoint, payload, label in endpoints:
            ...
            if latencies:
                ...
                print(
                    f"  [OK] {label}: {self.results['api_endpoints'][label]['mean_ms']:.2f}ms (+-{self.results['api_endpoints'][label]['stdev_ms']:.2f})"
                )
        ...
        for config in task_configs:
            ...
            try:
                ...
                resp = self.session.post(
                    RF_ACQUISITION_URL + config["endpoint"], json=config["payload"], timeout=TIMEOUT
                )
                        ...
                        while time.time() - start_time < TIMEOUT:
                            ...
                            if status_resp.status_code == 200:
                                ...
                                if status_data.get("state") in [
                                    ...
                                    print(
                                        f"    Completed in {elapsed:.2f}s (State: {status_data.get('state')})"
                                    )
                                    break
            ...
            if execution_times:
                ...
                print(
                    f"  [OK] {config['name']}: {self.results['celery_tasks'][config['name']]['mean_s']:.2f}s"
                )
    ...
    def benchmark_concurrent_capacity(self) -> None:
        ...
        def submit_concurrent_tasks():
                        ...
                        try:
                            ...
                            if resp.status_code == 200:
                                ...
                                if status in ["SUCCESS", "FAILURE", "PARTIAL_FAILURE"]:
                                    ...
                                    print(
                                        f"    Task {task_id[:8]}... completed: {completion_times[task_id]:.2f}s"
                                    )
                        except Exception:
                            pass
            ...
            self.results["concurrent_capacity"] = {
                ...
                "mean_completion_s": (
                    statistics.mean(completion_times.values()) if completion_times else 0
                ),
        ...
        if self.results["concurrent_capacity"]:
            ...
            print(
                f"  [OK] Concurrency: {cc['successful_completions']}/{cc['successful_submissions']} tasks completed"
            )
    ...
    def verify_inference_latency(self) -> None:
        ...
        try:
            ...
            latencies = []
            for _i in range(5):
                payload = {"mel_spectrogram": [[0.5] * 128] * 128}  # Dummy feature vector

                start = time.perf_counter()
                resp = self.session.post(
                    f"{inference_url}/api/v1/inference/predict", json=payload, timeout=TIMEOUT
                )
                elapsed = (time.perf_counter() - start) * 1000  # ms
    ...
    def generate_report(self, output_path: str) -> None:
        ...
        # Calculate summary
        api_means = [v["mean_ms"] for v in self.results["api_endpoints"].values()]
        if api_means:
        ...
        celery_means = [v["mean_s"] for v in self.results["celery_tasks"].values()]
        if celery_means:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", default="PHASE4_TASK_A3_PERFORMANCE_REPORT.md", help="Output report path"
    )
