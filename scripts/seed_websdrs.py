...
def fetch_health_check(base_url: str, timeout: int = 10) -> dict[str, Any] | None:
    """
...
def seed_websdrs():
    ...
    for websdr_config in WEBSDRS:
        ...
        for sdr in sdrs:
            ...
            logger.info(
                f"    - {sdr.get('name', 'N/A')} ({sdr.get('type', 'N/A')}): {len(profiles)} profiles"
            )

        # Upsert to database
        station_id = db_manager.upsert_websdr_from_health_check(
            name=name, url=base_url, health_data=health_data
        )
