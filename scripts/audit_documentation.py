...
class DocumentationAuditor:
        ...
        for match in re.finditer(md_pattern, content):
            ...
            try:
                ...
                if linked_file.exists() and str(linked_file).startswith(str(self.docs_path)):
                    ...
                    self.log(f"Broken link in {source_file.relative_to(self.base_path)}: {url}", 'warning')
            except Exception as e:
                self.log(f"Error resolving link '{url}' in {source_file.relative_to(self.base_path)}: {e}", 'warning')
        ...
        for entry_point in self.entry_points:
            if entry_point.exists():
                try:
                    ...
                    self.log(f"Entry point {entry_point.relative_to(self.base_path)} -> {len(links)} links")
                except Exception as e:
                    self.log(f"Error reading {entry_point.relative_to(self.base_path)}: {e}", 'error')
    ...
    def audit(self):
        ...
        for entry_point in self.entry_points:
            if entry_point.exists():
                ...
                self.log(f"Entry point not found: {entry_point.relative_to(self.base_path)}", 'warning')
    ...
    def generate_json_report(self, output_file: Path):
        ...
        report = {
            ...
            'broken_links': [
                {
                    'source_file': str(source.relative_to(self.base_path)),
                    'broken_url': url
                }
                for source, url in self.broken_links
            ],
            'entry_points': [str(ep.relative_to(self.base_path)) for ep in self.entry_points if ep.exists()]
        }
    ...
    def generate_markdown_report(self, output_file: Path):
        ...
        if self.orphaned_files:
            ...
            lines.extend([
                "## ✅ No Orphaned Files",
                "",
                "All documentation files are properly linked!",
                ""
            ])
