...
SERVICES = {
    ...
    "redis": {"host": "localhost", "port": 6379, "password": "changeme"},
    "minio": {
...
class HealthChecker:
    ...
    def check_rabbitmq(self):
        ...
        try:
            credentials = pika.PlainCredentials(
                SERVICES["rabbitmq"]["user"], SERVICES["rabbitmq"]["password"]
            )
    ...
    def check_prometheus(self):
        ...
        try:
            response = requests.get(
                f"{SERVICES['prometheus']['url']}/api/v1/status/runtimeinfo", timeout=5
            )
    ...
    def check_grafana(self):
        """Check Grafana API"""
        try:
            response = requests.get(f"{SERVICES['grafana']['url']}/api/health", timeout=5)
            if response.status_code == 200:
