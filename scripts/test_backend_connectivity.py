            ...
            if 200 <= response.status_code < 300:
                try:
                    ...
                    if isinstance(data, list):
                        print_status(
                            "✅", f"Status {response.status_code} OK", f"Received {len(data)} items"
                        )
                    else:
                        print_status(
                            "✅",
                ...
                print_status("❌", f"Status {response.status_code}", response.text[:200])
                return False
...
def main():
    ...
    print("   Testing API Gateway on localhost:8000")
