...
class LoadTest:
    ...
    def monitor_completion(self, task_ids: List[str]) -> None:
        ...
        while time.time() - monitoring_start < self.duration_seconds:
            ...
            for task_id in task_ids:
                if task_id in self.running_tasks and self.running_tasks[task_id]["state"] == "PENDING":
                    try:
            ...
            if current_time - last_checkpoint >= checkpoint_interval:
                ...
                print(f"  [TIME] {elapsed:.0f}s: {completed} completed, {pending} pending ({completed/len(task_ids)*100:.1f}%)")
    ...
    def analyze_results(self) -> None:
        ...
        self.results["metrics"] = {
            ...
            "completion_rate": len(self.completed_tasks) / len(self.running_tasks) * 100 if self.running_tasks else 0,
        ...
        if completion_times:
            self.results["metrics"]["completion_stats"] = {
                ...
                "p95_s": sorted(completion_times)[int(len(completion_times) * 0.95)] if completion_times else 0,
                "p99_s": sorted(completion_times)[int(len(completion_times) * 0.99)] if completion_times else 0,
            }
        ...
        if completion_times:
            ...
            print(f"  [OK] P95 Completion Time: {self.results['metrics']['completion_stats']['p95_s']:.2f}s")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="PHASE4_TASK_B1_LOAD_TEST_REPORT.md",
