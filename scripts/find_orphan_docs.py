    ...
    def print_detailed_report(self):
        ...
        for category, files in sorted(categories.items()):
            ...
            for orphan in sorted(files):
                ...
                if suggestions:
                    print("      💡 Suggested linking locations:")
    ...
    def generate_fix_plan(self, output_file: Path):
        ...
        for category, files in sorted(categories.items()):
            ...
            for orphan in sorted(files):
                ...
                lines.append("- **Action needed**: Review and link from:")
...
Examples:
  ...
  python scripts/find_orphan_docs.py --generate-plan # Create fix plan document
    ...
    parser.add_argument(
        "--detailed", action="store_true", help="Show detailed report with linking suggestions"
    )
    parser.add_argument(
        "--by-category", action="store_true", help="Show summary grouped by category"
    )
    parser.add_argument(
        "--generate-plan", action="store_true", help="Generate a markdown fix plan document"
    )
    ...
    if args.detailed:
        ...
        plan_file = BASE_PATH / "docs" / "agents" / "ORPHAN_FIX_PLAN.md"
