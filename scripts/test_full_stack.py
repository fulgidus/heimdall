...
def test_endpoint(name: str, url: str, expected_status: int = 200) -> bool:
    ...
    try:
        with httpx.Client(timeout=5.0) as client:
            ...
            print(f"   {status_color}{status_symbol} Status: {response.status_code}{Colors.END}", end="")
            if not status_ok:
            ...
            try:
                ...
                if isinstance(data, list):
                    print(f"   {Colors.GREEN}✅ Data Type: Array with {len(data)} items{Colors.END}")
                    if data:
        ...
        print(f"   Backend may be offline or unresponsive")
        return False
    except Exception as e:
...
def main():
    print(f"\n{Colors.BOLD}{'='*70}")
    print(f"🧪 Full Stack Test: Frontend → API Gateway → RF-Acquisition")
    ...
    if test_endpoint("Get WebSDRs (Direct Service)", "http://localhost:8001/api/v1/acquisition/websdrs"):
    ...
    if test_endpoint("Get WebSDRs (Via API Gateway)", "http://localhost:8000/api/v1/acquisition/websdrs"):
    ...
    if test_endpoint("Check WebSDR Health (Via Gateway)", "http://localhost:8000/api/v1/acquisition/websdrs/health"):
