...
class LockRequirementsManager:
    ...
    def run_safety_check(self, lock_file: Path) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["safety", "check", "-r", str(lock_file), "--json"],
                capture_output=True,
                text=True
            )
    ...
    def generate_report(self) -> bool:
        ...
        try:
            with open(md_report, 'w') as f:
                ...
                f.write(f"- **Vulnerabilities Found**: {len(self.audit_results['vulnerabilities'])}\n")
                ...
                if self.audit_results['files_processed']:
                    ...
                    for file_info in self.audit_results['files_processed']:
                        f.write(f"- `{file_info['file']}` → `{file_info['lock_file']}` "
                ...
                if self.audit_results['vulnerabilities']:
                    ...
                    for vuln in self.audit_results['vulnerabilities']:
                        f.write(f"### {vuln.get('package', 'Unknown')} - {vuln.get('severity', 'UNKNOWN')}\n")
...
def main():
    ...
    parser.add_argument(
        "--allow-unsafe",
        action="store_true",
        help="Allow unsafe packages (e.g., setuptools, pip)"
    )
    
    args = parser.parse_args()
    
    manager = LockRequirementsManager(
